...
for line in open("data/chs-eng-cut.txt", encoding='UTF-8').read().strip().split('\n'):
    src, trg = line.split('\t')
...
def src_pipeline(words):
    return [src_word2index.get(w, 0) for w in words]


def trg_pipeline(words):
    return [trg_word2index.get(w, 0) for w in words] + [EOS_TOKEN]
